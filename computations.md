...
In section \ref{sec:language} we described a computer algebra system to express and manipulate matrix expressions at a high, symbolic level.  This symbolic work is not appropriate for numeric computation.  In this section we describe a new isolated project, a separate system to define numeric codes at a high-level.  Later in section \ref{sec:matrix-compile} we will use compiler tools to connect these two to build a more cohesive whole.
